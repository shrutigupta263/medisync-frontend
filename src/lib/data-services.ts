...
export const reportsService = {
  ...
  async getUserReports(userId: string): Promise<UserReport[]> {
    try {
      ...
      // Fallback: Return mock data if database not set up
      console.warn('Database not set up, returning mock data:', error)
      return [
        {
          id: 'mock-1',
          user_id: userId,
  ...
  async getUserReport(userId: string, reportId: string): Promise<UserReport | null> {
    try {
      ...
      handleSupabaseError(error, 'fetch user report')
      return null
    }
  },

  // Create a new report for a user
  async createUserReport(userId: string, reportData: Partial<UserReport>): Promise<UserReport | null> {
    try {
      const { data, error } = await supabase
        .from('user_reports')
        .insert([{
      ...
      // Fallback: Return mock data if database not set up
      console.warn('Database not set up, returning mock report:', error)
      return {
        id: `mock-${Date.now()}`,
        user_id: userId,
        title: reportData.title || 'Uploaded Report',
